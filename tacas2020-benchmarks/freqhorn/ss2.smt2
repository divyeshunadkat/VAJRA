...
(rule (=> (and (inv3 a i n sum) (< i n) (= a1 (store a i 0)) (= i1 (+ i 1))) (inv3 a1 i1 n sum)))

(rule (=> (and (inv3 a i n sum) (>= i n)) (inv4 a 0 n sum)))

(rule (=> (and (inv4 a i n sum) (< i n) (= a1 (store a i (+ (select a i) sum))) (= i1 (+ i 1))) (inv4 a1 i1 n sum)))
...
(rule (=> (and (inv6 a i n sum) (< i n) (= a1 (store a i (+ (select a i) sum))) (= i1 (+ i 1))) (inv6 a1 i1 n sum)))

(rule (=> (and (inv6 a i n sum) (>= i n) (< 0 i1) (< i1 n) (not (= (select a i1) (* n (+ n 2))))) fail))
