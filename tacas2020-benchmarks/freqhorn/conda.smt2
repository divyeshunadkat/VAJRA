...
(rule (=> (and (inv2 a i n sum) (< i n) (= a1 (ite (= (select a i) 1) (store a i (+ (select a i) 1)) (store a i (- (select a i) 1)) )) (= i1 (+ i 1))) (inv2 a1 i1 n sum)))

(rule (=> (and (inv2 a i n sum) (>= i n)) (inv3 a 0 n sum)))

(rule (=> (and (inv3 a i n sum) (< i n) (= sum1 (+ sum (select a i))) (= i1 (+ i 1))) (inv3 a i1 n sum1)))

(rule (=> (and (inv3 a i n sum) (>= i n) (< 0 i1) (< i1 n) (not (= sum (* 2 n)))) fail))
