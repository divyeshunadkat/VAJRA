...
(rule (=> (and (inv2 a b i n sum) (< i n) (= sum1 (+ sum (select a i))) (= i1 (+ i 1))) (inv2 a b i1 n sum1)))

(rule (=> (and (inv2 a b i n sum) (>= i n)) (inv3 a b 0 n sum)))

(rule (=> (and (inv3 a b i n sum) (< i n) (= a1 (store a i (+ (select a i) sum))) (= i1 (+ i 1))) (inv3 a1 b i1 n sum)))

(rule (=> (and (inv3 a b i n sum) (>= i n)) (inv4 a b 0 n sum)))

(rule (=> (and (inv4 a b i n sum) (< i n) (= b1 (store b i (+ (select a i) 1))) (= i1 (+ i 1))) (inv4 a b1 i1 n sum)))

(rule (=> (and (inv4 a b i n sum) (>= i n) (< 0 i1) (< i1 n) (not (= (select b i1) (+ n 2)))) fail))
