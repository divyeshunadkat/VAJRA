...
(rule (=> (and (inv2 a b c i n sum) (< i n) (= b1 (store b i 1)) (= i1 (+ i 1))) (inv2 a b1 c i1 n sum)))

(rule (=> (and (inv2 a b c i n sum) (>= i n)) (inv3 a b c 0 n sum)))

(rule (=> (and (inv3 a b c i n sum) (< i n) (= c1 (store c i 1)) (= i1 (+ i 1))) (inv3 a b c1 i1 n sum)))

(rule (=> (and (inv3 a b c i n sum) (>= i n)) (inv4 a b c 0 n sum)))

(rule (=> (and (inv4 a b c i n sum) (< i n) (= sum1 (+ sum (select a i))) (= i1 (+ i 1))) (inv4 a b c i1 n sum1)))

(rule (=> (and (inv4 a b c i n sum) (>= i n)) (inv5 a b c 0 n sum)))

(rule (=> (and (inv5 a b c i n sum) (< i n) (= sum1 (+ sum (select b i))) (= i1 (+ i 1))) (inv5 a b c i1 n sum1)))
