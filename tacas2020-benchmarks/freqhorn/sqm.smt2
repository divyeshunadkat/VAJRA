...
(rule (=> (and (inv1 a b i n) (< i n)
    (= b1 (ite
      (= i 0)
      (store b i 1)
      (store b i (+ (select b (- i 1)) 2) )
    ))
    (= i1 (+ i 1))) (inv1 a b1 i1 n)))
...
(rule (=> (and (inv2 a b i n) (< i n)
    (= a1 (ite
      (= i 0)
      (store a i 1)
      (store a i (+ (select a (- i 1)) (select b (- i 1)) 2))
    ))
    (= i1 (+ i 1))) (inv2 a1 b i1 n)))

(rule (=> (and (inv2 a b i n) (>= i n) (< 0 i1) (< i1 n) (not (= (select a i1) (* (+ i1 1) (+ i1 1)) )) ) fail))
